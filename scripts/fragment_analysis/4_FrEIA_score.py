...
def mannwhitneyu_test(dat_Df, meta_df, args):
    ...
    new_data = pool.map(partial(mannwhitneyu_test_worker,
                                dat_Df,
                                args),
                                base_L)

    mwu_Df = pd.concat([mwu_Df] + new_data, ignore_index=True)
...
def euclidean_FrEIA_score(prop_df, div_df, control_V, case_V, args):
    ...
    new_data = pool.map(partial(euclidean_worker,
                                samp_df,
                                control_df,
                                case_df),
                                samp_L)

    ed_Df = pd.concat([ed_Df] + new_data, ignore_index=True)
