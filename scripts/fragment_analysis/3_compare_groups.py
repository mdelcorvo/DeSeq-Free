...
def CreateFolders(WhichLvl, args):
    if args.plot:
        if not os.path.exists("".join((args.ResultsFolder,
                                       "/4_Compare/Plots"))):
            os.makedirs("".join((args.ResultsFolder, "/4_Compare/Plots")
                                ))
    if not os.path.exists("".join((args.ResultsFolder,
                                   "/4_Compare/Data"))):
        os.makedirs("".join((args.ResultsFolder, "/4_Compare/Data")))
...
def ReadData(args, sampTDf, WhichGroup, prefix, lvl):
    ...
    for gr in WhichGroup:
        ...
        new_data = pool.map(partial(ReadFile,
                                    args,
                                    WhichGroup,
                                    prefix,
                                    gr,
...
def ThreadMDS(args, DataDf, groupBy, value):
    ...
    try:
        new_data = pool.map(partial(MotifDiversityScore,
                                    data,
                                    groupBy,
                                    value),
                                    GroupS)

        MDSDf = pd.concat([MDSDf] + new_data, ignore_index=True)
...
def GenomeLevelAnalysis(DataDf, sampTDf, Groups, ControlGroup,
    ...
    PlotOutPath = "".join((args.ResultsFolder, "/4_Compare/Plots/"))

    if prefix == "P__":   # START OF POLYNUCLEOTIDE ANALYSIS
        # Save data file.
        DataOutPath = "".join((args.ResultsFolder, "/4_Compare/Data/"))
