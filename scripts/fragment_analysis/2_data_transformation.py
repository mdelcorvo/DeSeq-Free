...
def MonoNcAnalysis(SampleDf, args, filename, group, lvl):
    ...
    # Save the results.
    BCDf.to_parquet("".join((args.output, "/3_Abundances/",
...
def PolyNcAnalysis(SampleDf, args, filename, group, lvl):
    ...
    # Save the results.
    OutDf.to_parquet("".join((args.output, "/3_Abundances/",
...
def JoinedEnds(SampleDf, args, filename, group, lvl):
    ...
    # Save the results.
    JMEDf.to_parquet("".join((args.output, "/3_Abundances/",
                              group, "/", lvl, "/JM__", filename, ".pq")),
                     compression="gzip",
                     index=False)
    JTEDf.to_parquet("".join((args.output, "/3_Abundances/",
...
def CreateFolders(WhichGroup, whichLvl, args):
    for lvl in whichLvl:
        if not os.path.exists("".join((args.output, "/3_Abundances/",
                                       WhichGroup, "/", lvl))):
            os.makedirs("".join((args.output, "/3_Abundances/",
                                 WhichGroup, "/", lvl)))
