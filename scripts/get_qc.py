...
output_dict = {}
with open(snakemake.input.stats, 'r') as f:
    for line in f:
        attrs = line.split()
...
res = pd.DataFrame(table,columns=["Pairs statistics", "Value", "Percentage"])  
#out_rep = open(snakemake.output.stats, "w")
#out_rep.writelines(res)
#out_rep.close()
res.to_csv(snakemake.output.stats, header=None, index=None, sep='\t', mode='w')
